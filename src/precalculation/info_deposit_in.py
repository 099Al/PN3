...
class InfoCalcToDep:
    ...
    def USD_DEPO_BTC(self):
        ...
        # на депозит
        x_depo = depof.X_to_DEPO(
            self.x_usd, self.u_dep_prc, self.u_dep_fix
        )  # перевод на депозит
        x_depo_sum = x_depo + self.resedual_usd
        btc_buy = transf.buyBTC(
            x_depo_sum, self.price_tobuy_btcusd, self.mk_tk_usd
        )  # покупка BTC максимальное кол-во
        btc = btc_buy["btc"]
    ...
    def USD_DEPO_BTC_v2(self):
        ...
        x_depo = depof.X_to_DEPO(
            rub, self.r_dep_prc, self.r_dep_fix
        )  # перевод на депозит
        x_depo_sum = x_depo + self.resedual_rub
        btc_buy = transf.buyBTC(
            x_depo_sum, self.price_tobuy_btcrub, self.mk_tk_rub
        )  # покупка BTC
        btc = btc_buy["btc"]
        x_resed = btc_buy["dx"]  # остаток X
    ...
    def RUB_DEPO_BTC(self):
        ...
        x_depo = depof.X_to_DEPO(
            self.x_rub, self.r_dep_prc, self.r_dep_fix
        )  # RUB на депозите
        x_depo_sum = x_depo + self.resedual_rub
        btc_r = transf.buyBTC(
            x_depo_sum, self.price_tobuy_btcrub, self.mk_tk_rub
    ...
    def RUB_DEPO_BTC_v2(self):
        ...
        x_depo = depof.X_to_DEPO(
            usd, self.u_dep_prc, self.u_dep_fix
        )  # перевод на депозит
        x_depo_sum = x_depo + self.resedual_usd
        btc0_u = transf.buyBTC(
            x_depo_sum, self.price_tobuy_btcusd, self.mk_tk_usd
        )  # покупка BTC
        btc = btc0_u["btc"]
    ...
    def X_DEPO_BTC(self, type="USD"):
        ...
        btc_buy = transf.buyBTC(
            x_depo_sum, price_tobuy_btc, mk_tk
        )  # покупка BTC максимальное кол-во
        btc = btc_buy["btc"]
        x_resed = btc_buy["dx"]  # остаток X
