...
import src.trade_utils.deposit_money as depof
import src.trade_utils.exchange as exchf
import src.trade_utils.trade as transf


# Комиссия ввод USD
com_in_pr_u = TradeConfig.u_dep_prc
com_in_f_u = TradeConfig.u_dep_fix
# Комиссия вывод USD
com_out_pr_u = TradeConfig.u_wthrw_prc
com_out_f_u = TradeConfig.u_wthrw_fix

# Комиссия ввод RUB
com_in_pr_r = TradeConfig.r_dep_prc
com_in_f_r = TradeConfig.r_dep_fix
# Комиссия вывод RUB
com_out_pr_r = TradeConfig.r_wthrw_prc
com_out_f_r = TradeConfig.r_wthrw_fix


limit_usd = TradeConfig.DEPO_LIMIT_IN_USD
limit_rub = TradeConfig.DEPO_LIMIT_IN_RUB
...
class InfoCalcFromDep:
    ...
    def USD_OUT(self):
        ...
        add_usd_d = 0
        if self.add_usd > 0:
            add_usd_d = depof.X_to_DEPO(
                self.add_usd, self.u_in_dep_prc, self.u_in_dep_fix
            )  # уйдет на депозит

        usd = self.x_usd + add_usd_d
        usd_out = depof.X_WITHD(
            usd, self.u_out_dep_prc, self.u_out_dep_fix
        )  # USD на карте
    ...
    def RUB_OUT(self):
        ...
        if self.add_rub > 0:
            add_rub_x = depof.X_to_DEPO(
                self.add_rub, self.r_in_dep_prc, self.r_in_dep_fix
            )

        rub = self.x_rub + add_rub_x
        rub_out = depof.X_WITHD(
            rub, self.r_out_dep_prc, self.r_out_dep_fix
        )  # RUB на карте
    ...
    def USD_btc_rub_OUT(self):
        ...
        btc = transf.buyBTC(usd, self.price_tobuy_btcusd, self.mk_tk_usd)
    ...
    def BTC_usd_OUT(self):
        ...
        u_out = depof.X_WITHD(
            usd_sum, self.u_out_dep_prc, self.u_out_dep_fix
        )  # На карте
    ...
    def X_OUT(self, type):
        ...
        if add_x > 0:
            add_x_dep = depof.X_to_DEPO(
                add_x, in_dep_pr, in_dep_fix
            )  # уйдет на депозит
        ...
        x_curr2 = exch_curr(
            x_out, exch_price2
        )  # Перевод X в Другую валюту по другому курсу
    ...
    def BTC_x_OUT(self, type):
        ...
        x = transf.sellBTC(btc_pre, price_tosell_btc, mk_tk)[
            "x"
        ]  # Получено при продаже
    ...
    def X_on_DEPOSIT_to_OUT_X0(self, x_out, type="RUB"):
        ...
        if type == "RUB":
            ...
            x_on_depo = transf.cutX(
                x_on_depo + 0.01, 2
            )  # поправка. чтобы не высчитывать округление берется наибольшее число
            return x_on_depo
    ...
    def X_on_DEPOSIT_Y_out(
        self, X, price_tobuy, price_tosell, type_x="RUB", type_y="RUB"
    ):
        """
        ...
        return {"x_depo": x_depo, "btc": btc1, "y_depo": y2, "y_out": y_out, "dp": dp}
